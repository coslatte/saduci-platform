...
const getRiskLevel = (probability: number) => {
  if (probability >= 0.7) return { label: 'High', color: 'text-red-600 bg-red-50' };
  if (probability >= 0.4) return { label: 'Medium', color: 'text-yellow-600 bg-yellow-50' };
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-3xl font-bold text-primary-600">{probabilityPercent}%</p>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg">
              ...
              {prediction.factors.slice(0, 5).map((factor, index) => (
                <div key={index} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-gray-900 font-medium">{factor.value.toFixed(2)}</span>
                    <span
