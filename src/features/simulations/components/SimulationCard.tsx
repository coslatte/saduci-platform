    ...
    <Card>
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[simulation.status]}`}>
              ...
              {simulation.results.slice(0, 5).map((result, index) => (
                <div key={index} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-gray-900 font-medium">{result.value.toFixed(2)}</span>
                    <span
